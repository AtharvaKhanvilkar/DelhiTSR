...
def resolve_smart_circle_valuation(data, meta, locality_category=None):
    """
    Deed-Derived Valuation Policy:
    NEVER calculate the property value formulaically.
    Only derive it directly from the deed if and when explicitly stated in the deed.
    Otherwise, return 0.0 (Dormant Formula Policy).
    """
    area_raw = (
    ...
    area_sqm = normalize_area_to_sqm(area_raw) if area_raw else 0.0

    # Derive ONLY if explicitly stated in deed text/recitals
    explicit_val = (
        ...
        try:
            ...
            if val_num > 0:
                return {
                    ...
                    "area_sqm": round(area_sqm, 2),
                    "valuation_source": "Explicit Deed Recital"
                }
        except Exception:
            pass

    # Formula calculations dormant per directive
    return {
        "circle_value": 0.0,
        "area_sqm": round(area_sqm, 2),
